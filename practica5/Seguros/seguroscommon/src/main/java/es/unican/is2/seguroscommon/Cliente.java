package es.unican.is2.seguroscommon;

import java.util.LinkedList;
import java.util.List;

/**
 * Clase que representa un cliente de la empresa de seguros
 * Un cliente se identifica por su dni
 */
public class Cliente {

    private String dni;

    private String nombre;  
    
    private boolean minusvalia;

    private List<Seguro> seguros = new LinkedList<Seguro>();

    public class SeguroYaContratado extends RuntimeException {

		public SeguroYaContratado() {
			super("Ya ha contratado este seguro.");
		}
		
	}

    public class NoHaySeguros extends RuntimeException {

		public NoHaySeguros() {
			super("El cliente no tiene seguros contratados.");
		}
		
	}

    public class SeguroNoExistente extends RuntimeException {

		public SeguroNoExistente() {
			super("El cliente no tiene ese seguro contratado.");
		}
		
	}
    
	/**
     * Retorna los seguros del cliente 
     */
    public List<Seguro> getSeguros() {
        return seguros;
    }
    
    /**
     * Asigna la lista de seguros
     */
    public void setSeguros(List<Seguro> seguros) {
        this.seguros = seguros;
    }

    /**
     * Retorna el nombre del cliente.   
     */
    public String getNombre() {
        return nombre;
    }

    /**
     * Asigna el nombre del cliente
     */
    public void setNombre(String nombre) {
        this.nombre = nombre;
    }
    /**
     * Retorna el dni del cliente.
     */
    public String getDni() {
        return dni;
    }

    /**
     * Asigna el dni del cliente
     * @param dni
     */
    public void setDni(String dni) {
        this.dni = dni;
    }
    
    /**
     * Indica si el cliente es minusvalido
     */
    public boolean getMinusvalia() {
    	return minusvalia;
    }

    /**
     * Asigna la minusvalia del cliente
     * @param minusvalia
     */
     public void setMinusvalia(boolean minusvalia) {
        this.minusvalia = minusvalia;
    }
    
    /**
     * Calcula el total a pagar por el cliente por 
     * todos los seguros a su nombre
     */
    public double totalSeguros() {
        double total = 0.0;
        for (Seguro s: seguros) {
            total += s.precio();
        }
        
        if (minusvalia) {
            total = total * 0.75;
        }

        return total;
    }

    public void anhadirSeguro (Seguro s) {
        if (s == null) {
            throw new NullPointerException();
        }
        for (Seguro s2: seguros) {
            if (s2.equals(s)) {
                throw new SeguroYaContratado();
            }
        }
        seguros.add(s);
    }

    public void quitarSeguro(Seguro s) {
        if (s == null) {
            throw new NullPointerException();
        }
        if (seguros.size() == 0) {
            throw new NoHaySeguros();
        }
        if (!seguros.contains(s)) {
            throw new SeguroNoExistente();
        }
        seguros.remove(s);
        
    }
}
